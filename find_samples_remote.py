...
for year in years:
  ...
  #samples['data'] = [i.replace('/hdfs', 'root://ndcms.crc.nd.edu/') for i in glob.glob(f'/hdfs/store/user/kaho/NanoPost_{year}_v2/SingleMuon/*/*/*/*root')]
    if samples_shorthand == 'WJetsToLNu_TuneCP5_13TeV-amcatnloFXFX-pythia8':
      samples['WJetsToLNu_TuneCP5'] = [i.replace('/hdfs', 'root://cmsxrootd.hep.wisc.edu/') for i in glob.glob(f'/hdfs/store/user/kaho/NanoPost_{year}_v2/{samples_shorthand}*/*/*/*/*root')]
    elif 'VBF_LFV' in samples_shorthand:
      samples[samples_shorthand.replace('_TuneCH3','H').replace('_TuneCP5','')] = [i.replace('/hdfs', 'root://cmsxrootd.hep.wisc.edu/') for i in glob.glob(f'/hdfs/store/user/kaho/NanoPost_{year}_v2/{samples_shorthand}*/*/*/*/*root')]
    else:
