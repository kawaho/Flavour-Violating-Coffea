...
def Rpt(lep1, lep2, jets=None):
    ...
    if jets==None:
        ...
        return (emVar + jets[0]).pt/(emVar.pt+jets[0].pt)
    elif len(jets)==2:
        return (emVar + jets[0] +jets[1]).pt/(emVar.pt+jets[0].pt+jets[1].pt)
    else:
        return -999
...
def interestingKin(emevents, Electron_collections, Muon_collections, MET_collections, Jet_collections, doSys=False, e_ss=None, year_='2017', jetUnc_=None,jetyearUnc=None,_jecYear=None):
    #make interesting variables
    #zero/any no. of jets
    emVar = Electron_collections + Muon_collections
    emevents["e_m_Mass"] = emVar.mass
    emevents["empt"] = emVar.pt
    emevents["empt_Per_e_m_Mass"] = emVar.pt/emevents["e_m_Mass"]
    ...
    emevents["abseEta"] = abs(emevents["eEta"])
    emevents["absmEta"] = abs(emevents["mEta"])
...
#     emevents["pZeta85"] = pZeta_ - 0.85*pZetaVis_
    ...
    emevents['absj1Eta'] = abs(emevents['j1Eta'])
    ...
    emevents['absj2Eta'] = abs(emevents['j2Eta'])
    ...
    if doSys:
      for UpDown in ['Up', 'Down']:
        ...
        tmpFac = numpy.ones(len(emevents))
        EBin = ak.where(abs(Electron_collections.eta)<1)
        EBout = ak.where((abs(Electron_collections.eta)>1)&(abs(Electron_collections.eta)<1.4442))
        EE = ak.where((abs(Electron_collections.eta)>1.566)&(abs(Electron_collections.eta)<2.5))
        tmpFac[EBin] = e_ss["UL-Photon-SS-Unc"].evaluate(year_,f"ss{UpDown.lower()}","EBin",Electron_collections[EBin].seedGain.to_numpy())
        tmpFac[EBout] = e_ss["UL-Photon-SS-Unc"].evaluate(year_,f"ss{UpDown.lower()}","EBout",Electron_collections[EBout].seedGain.to_numpy())
        tmpFac[EE] = e_ss["UL-Photon-SS-Unc"].evaluate(year_,f"ss{UpDown.lower()}","EE",Electron_collections[EE].seedGain.to_numpy())
...
#        tmpElectron_collections = tmpElectron_collections*tmpElectron_collections[f'dEscale{UpDown}']/tmpElectron_collections.energy
        ...
        emevents[f'e_m_Mass_ess_{UpDown}'] = tmpemVar.mass
        #emevents[f"empt_ess_{UpDown}"] = tmpemVar.pt
        emevents[f"empt_Per_e_m_Mass_ess_{UpDown}"] = tmpemVar.pt/emevents[f'e_m_Mass_ess_{UpDown}']
        emevents[f"Rpt_ess_{UpDown}"] = Rpt(Muon_collections, tmpElectron_collections, [Jet_collections[:,0], Jet_collections[:,1]])
        emevents[f"pt_cen_Deltapt_ess_{UpDown}"] = pt_cen(Muon_collections, tmpElectron_collections, [Jet_collections[:,0], Jet_collections[:,1]])/(Jet_collections[:,0] - Jet_collections[:,1]).pt
...
#        emevents[f"pt_cen_Deltapt_eer_{UpDown}"] = pt_cen(Muon_collections, tmpElectron_collections, [Jet_collections[:,0], Jet_collections[:,1]])/(Jet_collections[:,0] - Jet_collections[:,1]).pt
        ...
        emevents[f"empt_Per_e_m_Mass_me_{UpDown}"] = tmpemVar.pt/emevents[f'e_m_Mass_me_{UpDown}']
        ...
        for jetUnc in jetUnc_+jetyearUnc:
           if jetUnc in jetyearUnc and not _jecYear in jetUnc:
             #ignore this year
             ...
             if 'jer' in jetUnc: 
               ...
               jetUncNoyear=jetUnc
             if sum(emevents["year"])==0:
               semitmpJet_collections[f'passJet30ID_{jetUnc}{UpDown}'] = (Electron_collections.delta_r(semitmpJet_collections) > 0.4) & (Muon_collections.delta_r(semitmpJet_collections) > 0.4) & (semitmpJet_collections[f'pt_{jetUncNoyear}{UpDown}']>30) & ((semitmpJet_collections.jetId>>1) & 1) & (abs(semitmpJet_collections.eta)<4.7) & ((semitmpJet_collections.puId>>2) | (semitmpJet_collections[f'pt_{jetUncNoyear}{UpDown}']>50))  
             else:
...
#             emevents[f'absj1Eta_{jetUnc}_{UpDown}'] = abs(tmpJet_collections[:,0].eta)
