...
#Separate into a dataframe per cat
df_ggcat0, df_ggcat1, df_ggcat2, df_vbfcat = df[(df['isVBFcat']==0)&(df['isHerwig']==0)&(df['nJet30']==0)], df[(df['isVBFcat']==0)&(df['isHerwig']==0)&(df['nJet30']==1)], df[(df['isVBFcat']==0)&(df['isHerwig']==0)&(df['nJet30']>1)], df[(df['isVBFcat']==1)&(df['isHerwig']==0)]
...
df_ggcat0_data, df_ggcat1_data, df_ggcat2_data, df_vbfcat_data = df_data[(df_data['isVBFcat']==0)&(df_data['nJet30']==0)], df_data[(df_data['isVBFcat']==0)&(df_data['nJet30']==1)], df_data[(df_data['isVBFcat']==0)&(df_data['nJet30']>1)], df_data[(df_data['isVBFcat']==1)]

#Create dataframe for fast calulcations of systematics
for df_gg_vbf, df_gg_vbf_data, whichcat in zip([df_ggcat0, df_ggcat1, df_ggcat2, df_vbfcat], [df_ggcat0_data, df_ggcat1_data, df_ggcat2_data, df_vbfcat_data], ['GGcat0', 'GGcat1', 'GGcat2', 'VBFcat']):
...
#        dict_.append([f'weight_herwig', subdf[f'weight'].sum()])
      ...
      for UpDown in ['Up', 'Down']:
        ...
        for sys in jetUnc+jetyearUnc:
          if whichcat=='GGcat0':
            df_gg_vbf_deep_sys = df[(df[f'isVBFcat_{sys}_{UpDown}']==0) & (df[f'isVBF']==(whichcat_deep=='VBF')) & (df['isHerwig']==0) & (df[f'nJet30_{sys}_{UpDown}']==0)] 
          elif whichcat=='GGcat1':
            df_gg_vbf_deep_sys = df[(df[f'isVBFcat_{sys}_{UpDown}']==0) & (df[f'isVBF']==(whichcat_deep=='VBF')) & (df['isHerwig']==0) & (df[f'nJet30_{sys}_{UpDown}']==1)] 
          elif whichcat=='GGcat2':
            df_gg_vbf_deep_sys = df[(df[f'isVBFcat_{sys}_{UpDown}']==0) & (df[f'isVBF']==(whichcat_deep=='VBF')) & (df['isHerwig']==0) & (df[f'nJet30_{sys}_{UpDown}']>1)] 
          else: 
            df_gg_vbf_deep_sys = df[(df[f'isVBFcat_{sys}_{UpDown}']==1) & (df[f'isVBF']==(whichcat_deep=='VBF')) & (df['isHerwig']==0)] 
...
#    result = pool.map(FillSys, [i for i in range(len(quantiles)-1)])
#    print(result)
